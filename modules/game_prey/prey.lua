...
function terminate()
	...
	if preyButton and not preyButton:isDestroyed() then
		preyButton:destroy()
		preyButton = nil
	end
	if preyTrackerButton and not preyTrackerButton:isDestroyed() then
	...
	if msgWindow and not msgWindow:isDestroyed() then
		msgWindow:destroy()
		msgWindow = nil
	end
end
...
function check()
	...
	if g_game.getFeature(GamePrey) then
		if not preyButton then
			preyButton = modules.game_mainpanel.addToggleButton('preyButton', tr('Prey Dialog'), '/images/options/button_preydialog', toggle, false, 7)
			if preyButton then
		...
		if not preyTrackerButton then
			preyTrackerButton = modules.game_mainpanel.addToggleButton('preyTrackerButton', tr('Prey Tracker'), '/images/options/button_prey', toggleTracker, false, 8)
...
function onResourcesBalanceChange(balance, oldBalance, type)
	...
	local totalGold = player:getTotalMoney()
	preyWindow.gold:setText(comma_value(tostring(totalGold)))

	-- We also update wildcards if the client supports it
	local wildCards = 0
	if ResourceTypes and ResourceTypes.PREY_WILDCARDS then
		wildCards = player:getResourceBalance(ResourceTypes.PREY_WILDCARDS)
	else
		wildCards = bonusRerolls
	end
	preyWindow.wildCards:setText(tostring(wildCards))
end
